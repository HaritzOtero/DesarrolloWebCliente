...
export class TransactionService {
  ...
  async addTransaction(transaction: Transaction) {
    //insert-a;
    const query =
      'INSERT INTO pending_transactions (endpoint, method, payload) VALUES (?,?,?)';
    this.storage
      .executeSql(query, [
  ...
  async removeTransaction(transaction: Transaction) {
    ...
    this.storage
      .executeSql(query, [transaction.id])
      .catch((error) =>
        console.error('Errorea transakzio bat ezabatzen', error)
      );
  }
  ...
  async removeAllTransactions() {
    ...
    this.storage
      .executeSql(query)
      .catch((error) =>
        console.error('Errorea transakzio guztiak ezabatzen', error)
      );
  }
}
